...
const getEnvVar = (key: string, defaultValue: string = '') => {
  try {
    // Access import.meta.env directly with proper error handling
...
class PerformanceMonitor {
    ...
    try {
      ...
      const observer = new PerformanceObserver((list) => {
        list.getEntries().forEach((entry) => {
          ...
          // Report critical metrics in production
  ...
  private reportMetric(name: string, value: number) {
    ...
    if (ENV.isProduction && ENV.enableAnalytics && typeof window !== 'undefined' && (window as any).gtag) {
  ...
  trackPageLoad(page: string) {
    ...
    return () => {
      ...
      // Report to analytics
      if (ENV.enableAnalytics && typeof window !== 'undefined' && (window as any).gtag) {
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Comprehensive meta tags for production
    ...
    const metaTags = [
      ...
      { property: 'og:image', content: `${ENV.siteUrl}/og-image-1200x630.png` },
      { property: 'og:image:width', content: '1200' },
      { property: 'og:image:height', content: '630' },
      { property: 'og:image:alt', content: 'Aayush Pawar - Digital Marketing Expert & AI Innovation Leader' },
      { property: 'og:site_name', content: ENV.siteName },
      { property: 'og:locale', content: 'en_US' },
      
      // Twitter Card
      { name: 'twitter:card', content: 'summary_large_image' },
      { name: 'twitter:title', content: 'Aayush Pawar | Digital Marketing Expert & AI Innovation Leader' },
      { name: 'twitter:description', content: 'Digital First Marketer, AI Specialist & Creative Visionary with 9+ years expertise in healthcare marketing, brand campaigns, and technology innovation.' },
      { name: 'twitter:image', content: `${ENV.siteUrl}/twitter-image-1024x512.png` },
      { name: 'twitter:image:alt', content: 'Aayush Pawar Portfolio - Raleskip' },
      { name: 'twitter:creator', content: '@raleskip' },
      { name: 'twitter:site', content: '@raleskip' },
      ...
      "@graph": [
        {
          ...
          "name": "Aayush Pawar",
          "alternateName": "Raleskip",
          "description": "Digital First Marketer, AI Specialist & Creative Visionary with 9+ years expertise in healthcare marketing, brand campaigns, and technology innovation.",
          "url": ENV.siteUrl,
        ...
        },
        {
          ...
          "url": ENV.siteUrl,
          "name": ENV.siteName,
          "description": "Digital marketing expert and AI innovation leader portfolio showcasing 9+ years of expertise in strategic marketing and creative solutions.",
          "publisher": { "@id": `${ENV.siteUrl}/#person` }
        }
      ]
    }
    ...
    if (!canonicalLink) {
      ...
      canonicalLink.href = ENV.siteUrl
      document.head.appendChild(canonicalLink)
    }

    log('[SEO] Meta tags and structured data initialized')
  }, [])
  ...
  useEffect(() => {
    if (ENV.enableAnalytics && ENV.gaId && ENV.isProduction && typeof document !== 'undefined') {
      ...
      ;(window as any).gtag('config', ENV.gaId, {
  ...
  const backgroundParticles = useMemo(() => {
    ...
    const getParticleCount = () => {
      ...
      if (window.innerWidth < 1200) return 10 // Tablet
      return 14 // Desktop
    }
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      ...
      if (ENV.enableAnalytics && (window as any).gtag) {
    ...
    if ('connection' in navigator) {
      ...
      const updateConnectionInfo = () => {
        log(`[Network] Connection: ${connection.effectiveType}, Downlink: ${connection.downlink}Mbps`)
      }
    ...
    const updateProgress = async () => {
      ...
      if (!nextStageData) {
        ...
        if (isActive) {
          ...
          if (ENV.enableAnalytics && typeof window !== 'undefined' && (window as any).gtag) {
            (window as any).gtag('event', 'app_loaded', {
  ...
  const handlePageChange = useCallback((page: Page) => {
    ...
    if (ENV.enableAnalytics && typeof window !== 'undefined' && (window as any).gtag) {
      (window as any).gtag('config', ENV.gaId, {
        page_title: `${page.charAt(0).toUpperCase() + page.slice(1)} - ${ENV.siteName}`,
        page_location: `${ENV.siteUrl}#${page}`,
      })
      
      ;(window as any).gtag('event', 'page_view', {
        page_title: `${page.charAt(0).toUpperCase() + page.slice(1)} - Raleskip`,
        page_location: `${ENV.siteUrl}#${page}`,
        page_referrer: `${ENV.siteUrl}#${previousPage}`,
        session_id: performanceMonitor.getSessionId()
      })
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      const pageTitle = `${page.charAt(0).toUpperCase() + page.slice(1)} - ${ENV.siteName}`
  ...
  const KineticText = useCallback(({ 
    ...
    // Convert children to string and handle edge cases
    const textContent = React.Children.toArray(children).join('')
    
    // Validate that we have a string to work with
    if (typeof textContent !== 'string' || !textContent.trim()) {
      return <span className={className}>{children}</span>
    }

    // Skip animation for very long text or when motion is reduced
...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('error', (event) => {
    ...
    if (ENV.isProduction && ENV.enableAnalytics && (window as any).gtag) {
      (window as any).gtag('event', 'exception', {
  ...
  if ('PerformanceObserver' in window) {
    const observer = new PerformanceObserver((list) => {
      list.getEntries().forEach((entry) => {
        ...
        if (ENV.isProduction && ENV.enableAnalytics && entry.duration > 100 && (window as any).gtag) {
